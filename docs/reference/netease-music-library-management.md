...
关闭 Document 解除订阅、取消草稿读取，已接纳 mutation 由共享协调器收口；最后一个音乐 Document 关闭会停止播放并释放原生音频资源。插件 Shutdown 先结束播放资源会话，再关闭库服务与账号 / 播放依赖。取消资格与异步完成任务保证正常收口；即使外部测试适配忽略取消，也不阻塞库协调器关闭，迟到结果不能发布。
