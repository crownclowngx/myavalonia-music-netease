...
歌曲搜索每页 30 条，关键词去除两端空白，长度 1–200，offset 0–30000。V4.1 已新增只读歌单浏览，见[日常播放器当前实现](netease-daily-player.md)；队列、歌词、拖动进度、下载收藏或更高音质请求尚未接入界面。结果展示名称、歌手、专辑；播放详情展示封面、时长、试听标记和状态。封面失败显示音符，不影响歌曲。

一个插件容器共享一个账号和单曲播放器，每个 Document 独立搜索。播放由最后发起的 Document 拥有；关闭其他 Document 不停止，真正关闭所有者停止。Tool 是右侧 singleton，关闭行为为 Hide。暂时卸载 View 只释放图片和订阅，不取消音乐。
...
每次 Play 先登记新代次，再取消旧工作，等待其停止和文件释放后执行新请求。详情、地址、缓冲、引擎打开均检查撤销；A→B→C 最终只允许 C 接管。暂停令牌与原播放代次关联，音量继承和用户修改共用顺序。进度只读，不用 UI 计时器伪造。
...
- 仅 HTTP 410 明确过期时重新解析地址一次；普通 403 不重试。无地址直接提示当前账号暂不可播放。
...
Windows 额外为本插件私有 LibVLCSharp 程序集绑定选定 DLL 句柄，避免同名原生库导入漂移。绑定是程序集级不可热换事实，引擎和播放状态仍由容器独立拥有；不设置 PATH、VLC_PLUGIN_PATH 或全局 DLL 搜索目录，不卸载其他插件库。两个插件同时运行及不同加载顺序仍属于真实 Host 待验收项，不能用单独复用视频目录的探针代替。
