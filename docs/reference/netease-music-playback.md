...
一个插件容器共享一个账号、队列和单曲执行器，每个 Document 独立搜索。播放资源由音乐 Document 租约共同持有：最后一个关闭时取消加载和自动切歌、保留续播位置，串行释放 MediaPlayer、Media、缓冲文件和 LibVLC 引擎；重开后点击播放才重建引擎，旧会话必须先完成释放。仍有其他音乐 Document 时继续共享播放。Tool 是右侧 singleton，关闭行为为 Hide，不持有播放租约。暂时卸载 View 只释放图片和订阅，不取消音乐。原生 DLL 绑定保持首次目录，释放引擎不执行 FreeLibrary，也不允许热换目录。
