...
> 状态：上游源码/文档调研完成；C# 接入未实施，真实账号和全部接口均未实测。核对日期：2026-09-22。
...
| 优先级 | 范围 | 状态 |
| --- | --- | --- |
| 登录最短路径 | QR key、本地二维码、扫码状态、账号检查、Cookie 会话、退出 | 本计划 P0–P3；待实施 |
...
实施时为实际用到的模块维护下表；本表为初始计划，不是已接入清单。完整权威映射在落地后移入 reference，本调研文档保留固定基线。

| 上游模块 | 计划业务方法 | 协议 | C# 实施 | 自动验证 | 真实账号验证 |
| --- | --- | --- | --- | --- | --- |
| login_qr_key | 创建二维码登录凭据 | eapi（当前默认配置） | 未实施 | 未执行 | 未执行 |
| login_qr_create | 本地构造二维码内容 | 本地 | 未实施 | 未执行 | 未执行 |
| login_qr_check | 查询扫码状态 | eapi（当前默认配置） | 未实施 | 未执行 | 未执行 |
| login_status | 检查当前账号 | weapi | 未实施 | 未执行 | 未执行 |
| logout | 请求远端退出 | eapi（当前默认配置） | 未实施 | 未执行 | 未执行 |
| register_anonimous | 游客初始化（是否纳入由 P0 决定） | xeapi | 未实施 | 未执行 | 未执行 |
