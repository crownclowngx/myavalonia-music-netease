...
具体实施以[V4 方案](netease-v4-m2-daily-player-plan.md)为准，验证方法见[V4 / M2 矩阵](../maintenance/netease-v4-m2-daily-player-verification.md)。V4 沿用完整临时缓冲，先验证本地定位；将 M1 的“关闭播放所有者停止”迁移为“页面关闭继续，账号退出或 Host 停止收口”。这些是目标行为，当前代码仍遵循 M1 契约。
...
按 `afb3159` 源码重新核对。下表区分 V2/V3 已有能力与 V4 待实施工作，不再把 M1 已完成的基础设施列作缺口。
...
每阶段随代码补测试；完整 M2 完成要求自动与实际播放/Host 证据齐全。当前仅编写实施指导，不提前标记功能或验证通过。
