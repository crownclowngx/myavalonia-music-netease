...
> 更新日期：2026-09-22。代码基线：`c469f919e931e60d30797cbd357d245e84e7cf01`。
> 起点：用户已确认登录没有问题，后续以登录可用为前提推进音乐能力。
> 状态：能力规划，M1–M5 尚未实施；里程碑编号不代表插件版本或交付日期。
...
| 账号与会话 | 账号核验、昵称和头像、单账号状态、DPAPI 保存/恢复、重试保存、退出清理均有实现 | 音乐业务需要新增受控的会话访问方式，不能从界面读取 Cookie |
| 真实验证 | 微信授权、网易回调和账号核验已有成功记录；本次用户确认登录可用 | 历史记录中恢复、远端退出、Host 验证仍有未覆盖项，随下一阶段联调补记，不作为重新开启登录开发的理由 |
| 网络与协议 | 原生 C# + Flurl，已实现 eapi/weapi、请求级凭据、取消、有限重试策略和响应大小限制 | 尚无 xeapi；已有传输和异常语义主要服务于登录 |
| 音乐数据 | 尚无搜索、歌曲详情、歌词、用户歌单服务 | 需要按用户场景增加类型化接口、分页和展示状态 |
| 本地播放 | 尚无音频引擎、媒体流、进度、队列、音量或输出设备管理 | 获取播放地址之后，仍需完整实现本地播放链路 |
| 页面与宿主 | 当前只有登录主页面；Plugin 与 Standalone 复用服务和界面；已接入插件生命周期 | 需要音乐工作区、共享播放器和 Host 内资源释放验证 |
...
- **先交付 Windows x64。** 当前凭据保护、插件包和交付目标均按此范围设计；其他系统需要另外验证保护实现、音频组件与部署。
- **沿用 C# + Flurl 的接入方式。** 业务 HTTP、媒体下载和缓冲保持明确所有权；音频引擎选择必须验证实际取流方式。
- **本轮是路线图文档。** 这里列出的功能、技术名称和验收目标均不表示已经实现或实测通过。
...
1. 当前编码器仅支持 eapi/weapi；固定上游的 `song_url_v1` 使用 xeapi，需要核对请求核心、初始化依赖、编解码和会话适配，并完成最小验证。是否可用及需要哪些依赖，以接入结果为准。[固定模块源码](https://raw.githubusercontent.com/NeteaseCloudMusicApiEnhanced/api-enhanced/a8c781fd64faab17fedfd46e0615a2609307f163/module/song_url_v1.js)
2. 先使用受控音频样本验证候选引擎，再用真实网易返回资源验证“取流 → 解码 → 输出”。检查流/本地缓冲输入、取消、停止、格式支持、原生依赖与 Host 加载；通过后再固定引擎选型。当前不预先承诺某个播放库。
...
| 插件依赖交付 | 当前私有包只有登录所需依赖，没有音频原生资产 | 选型时验证 win-x64 原生资产、Host 插件隔离与私有依赖交付；继续通过公开 SDK 接入，不引用 Host 内部工程 |
