...
> 状态：实施中；2026-09-22 已完成首批协议/Flurl 适配和无历史凭据的最小联网验证，P1–P3 继续推进。原始计划基线保留供对照，当前进展见[阶段记录](../archive/records/netease-v1/p0-protocol-verification-20260922.md)。
> 本仓调研基线：`1d58e97a4f65a915f3dabc0318837b491a645bc5`；编写前工作树干净。实施前重新检查工作树。
> 配套：[专用开发验证计划](netease-v1-login-verification.md)、[上游能力清单](netease-api-enhanced-capabilities.md)。
...
打开网易云音乐页面 → 点击扫码登录 → 本地显示二维码 → 使用网易云音乐 App 扫码并确认 → 页面显示账号 ID、昵称和头像 → 关闭后重开可检查并恢复会话 → 主动退出后清除本地登录信息。
...
| 6 | `logout` / `/logout` | POST `/api/logout`，随后完成本地退出 | 默认 eapi |
...
800–803 是响应正文的业务状态，不是 HTTP 状态。建议初始轮询间隔 2 秒、单次 HTTP 超时 15 秒、本地尝试预算 3 分钟；这些是待实测调整的客户端策略，不是网易承诺的二维码有效期。800 立即优先终止；不使用重叠定时回调；一次响应处理完毕后才等待下一轮。
...
实施时沿用邻仓 Flurl.Http 4.0.2 作为候选统一版本，先核对 net10.0 与插件依赖约束；本次不改包版本。二维码、凭据保护与后续 xeapi 加密库按真实需要选择，不引入 Node 运行时。
...
详细要求与编号见[专用验证计划](netease-v1-login-verification.md)。后续实现需要具备：
...
本仓目前没有专用开发门禁脚本；实施时提供一个薄的本地入口编排构建、测试和文档检查。不得假定 host 门禁会替本插件验证账号业务，也不引入 Windows CI 作为开发完成条件。
...
| docs/README.md | 本插件唯一文档总导航，区分当前模板能力与待实施计划 |
| docs/roadmap/ | 本方案、专用验证计划、上游能力调研与待办；全部标记状态及日期 |
...
- [ ] P0：全新目录验证最短 QR 链，确认匿名初始化依赖。
- [ ] P1：Flurl 接入、eapi/weapi、登录状态和会话存储。
- [ ] P2：登录页面与关闭/取消/恢复/退出交互。
- [ ] P3：补齐专项测试、本地开发门禁和现行使用文档。
