# 网易云音乐 V1：扫码登录专用开发验证计划

> 用途：为[Flurl 与登录方案](netease-v1-flurl-login-plan.md)提供可追溯的测试矩阵、本地开发门禁和人工验证范围。
> 状态：待实施、未执行；核对日期：2026-09-22。以下编号为计划中的场景编号，不是已有测试名称，也不表示测试通过。
...
后续实施提供薄的 `tools/verify-development.ps1` 入口（**当前不存在，本文不创建它**），复用以下标准命令，并检查每个退出码及 TRX 完整性。入口只做本仓开发验证，不调 host 发布工具。

以下命令在本仓根目录执行，是实施后的开发命令示意，本次未执行：
...
3. 检查当前模板、待实施计划和验证状态的表述一致。
4. 运行 `git diff --check`，确认修改仅涉及预期 Markdown。
5. 不执行应用构建、单测、账号登录、安装部署、Windows CI 或发布门禁。

本次计划编写不构成任何 P/H/L/S/U/M 场景通过的证据。实施完成后，将本文件中的可复用回归部分按 host 惯例迁至 maintenance，执行结果放在带日期的 archive/records，更新所有入口。
