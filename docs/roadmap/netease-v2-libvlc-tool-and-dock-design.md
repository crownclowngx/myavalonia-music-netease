...
多个插件共用目录意味着复用同一组原生文件，不意味着共享账号、音量、播放状态、LibVLC 实例或 MediaPlayer。音乐插件在自己的容器中惰性创建一个 LibVLC，始终只有一个活动播放器；实现按歌曲代次重建 MediaPlayer/Media，以隔离旧事件。Tool 与所有音乐 Document 观察同一套服务状态。
...
| 账号操作 | 引导从“新建”打开既有登录 Document；提供恢复、重试保存和退出，复用唯一登录协调器 |
| 运行库设置 | 手输/选择目录、检测、保存、清空配置；未登录时同样可操作 |
| 当前状态 | 内置/指定来源、配置目录与实际目录、检测问题、重启待生效；不显示协议密钥或 Cookie |

Tool 不持有另一份 AuthContext，也不从磁盘自行恢复账号。实施核对 SDK 3.4.1 无运行时打开指定 Document 的公开端口，因此显示“新建”引导，不引入 Host 内部依赖。Tool 发起退出时使用自己的稳定操作所有者，并沿用共享账号撤销逻辑。
...
Standalone 以音乐/设置页签复用真实 Document/Tool 模型和 View；登录与音乐在同一 Document 中。没有第二套账号设置实现，也不能把页签测试当作真实 Dock 验收。
...
原生事件只投递缓存事实，不在回调中重入 Time/Length、同步 Stop、Dispose 或等待 UI。停止完成后解除 Media 引用、释放当前播放器与媒体句柄，再删临时音频；容器最终结束才释放复用引擎。普通停止不释放共享库文件，更不调用跨插件 DLL 卸载。
