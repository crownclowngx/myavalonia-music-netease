...
public sealed record RawLyrics(string? Lrc, bool Instrumental = false, string? Translation = null, string? Yrc = null, string? YTranslation = null);
...
public sealed record LyricDocument(IReadOnlyList<LyricLine> Lines, string PlainText, string Status, string UnmatchedTranslation = "")
{
...
{
    public static int FindWord(IReadOnlyList<LyricWord> words, long positionMs)
    {
        var lo = 0; var hi = words.Count - 1;
        while (lo <= hi) { var mid = lo + (hi - lo) / 2; if (words[mid].StartMs <= positionMs) lo = mid + 1; else hi = mid - 1; }
        return hi >= 0 && positionMs < words[hi].StartMs + words[hi].DurationMs ? hi : -1;
    }
