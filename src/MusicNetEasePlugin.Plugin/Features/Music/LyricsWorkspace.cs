...
public sealed class LyricsWorkspace : ObservableObject, IDisposable
{
    ...
    private void Apply(LyricsSnapshot snapshot)
    {
        ...
        _snapshot = snapshot;
        if (previous >= 0 && previous < Lines.Count) Lines[previous].IsCurrent = false;
        if (CurrentLine >= 0 && CurrentLine < Lines.Count) Lines[CurrentLine].IsCurrent = true;
        foreach (var property in new[] { nameof(CurrentLine), nameof(Message), nameof(PlainText) }) OnPropertyChanged(property);
