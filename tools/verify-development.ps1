...
[CmdletBinding()]
param([ValidateSet('Login', 'M1', 'V3', 'V4', 'V5')][string]$Milestone = 'V5')
...
try {
    ...
    $native = if ($Milestone -in @('M1','V3','V4','V5')) { Get-Content -LiteralPath (Join-Path $results 'libvlc-offline.json') -Raw | ConvertFrom-Json } else { $null }
    @{ runId = $runId; revision = $source.revision; workingTreeDirty = $source.workingTreeDirty; sourceSha256=$source.sourceSha256; testsPassed = $passed;
        ...
        milestone = $Milestone; scenarioCoverage = $coverage; uiScenarioCoverage = $uiCoverage; m2ScenarioCoverage=$m2Coverage; v5ScenarioCoverage=$v5Coverage; v5StaticAssets=$v5Assets; artifacts=$artifacts;
