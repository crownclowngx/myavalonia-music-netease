...
[CmdletBinding()]
param([ValidateSet('Login', 'M1', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8')][string]$Milestone = 'V8')
...
try {
    ...
    $coverage = $null
    if ($Milestone -in @('M1','V3','V4','V5','V6','V7','V8')) {
    ...
    $uiCoverage = $null
    if ($Milestone -in @('V3','V4','V5','V6','V7','V8')) {
    ...
    $m2Coverage = $null
    if ($Milestone -in @('V4','V5','V6','V7','V8')) {
    ...
    $v8Coverage = $null
    if ($Milestone -eq 'V8') {
        $v8Coverage = Assert-V8TestMap (Join-Path $results 'netease-login-development.trx') (Join-Path $PSScriptRoot 'v8-test-map.json')
        Assert-V8Artifacts $results $context $started
    }
    ...
    $native = if ($Milestone -in @('M1','V3','V4','V5','V6','V7','V8')) { Get-Content -LiteralPath (Join-Path $results 'libvlc-offline.json') -Raw | ConvertFrom-Json } else { $null }
    @{ runId = $runId; revision = $source.revision; workingTreeDirty = $source.workingTreeDirty; sourceSha256=$source.sourceSha256; testsPassed = $passed;
        ...
        milestone = $Milestone; scenarioCoverage = $coverage; uiScenarioCoverage = $uiCoverage; m2ScenarioCoverage=$m2Coverage; v5ScenarioCoverage=$v5Coverage; v6ScenarioCoverage=$v6Coverage; v7ScenarioCoverage=$v7Coverage; v8ScenarioCoverage=$v8Coverage; v5StaticAssets=$v5Assets; artifacts=$artifacts;
